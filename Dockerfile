...
RUN if [ "$Configuration" = "master" ]; then npm run ng build -- --configuration  master --prod --output-path=dist; else then npm run ng build -- --configuration  realese --prod --output-path=dist; fi;
