...
RUN if [ "$Configuration" = "master" ]; then npm run ng build -- --configuration  echo master --prod --output-path=dist; else then npm run ng build -- --configuration  echo realese --prod --output-path=dist; fi;
